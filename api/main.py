...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # read from predictor.ml_predictor (= pipeline).
        ...
        if _segment_router is not None and getattr(_segment_router, "enabled", False):
            ...
            if _specialist_name:
                logger.info(f"       Verifying specialist model checksum: {_specialist_name}")
                _verify_model_checksums(model_path, active_model=_specialist_name)
        else:
            logger.info("       Specialist router not active — skipping specialist checksum")
        ...
        _do_warmup: bool = (
            _env_raw.lower() not in ("false", "0", "no") if _env_raw is not None else _cfg_warmup
        )
        logger.info(
